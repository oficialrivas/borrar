...
async def run(playwright, nit):
    ...
    await page.select_option("#tipo_busqueda", "nit")
    await page.fill("input[name='buscador']", nit)
...
async def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Se requiere un número de NIT."}, indent=4, ensure_ascii=False))
        sys.exit(1)

    nit = sys.argv[1]  # Toma el NIT desde la línea de comandos

    async with async_playwright() as playwright:
        await run(playwright, nit)
